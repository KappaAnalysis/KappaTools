...
namespace KappaTools
{
	...
	StandardJetPlots<JetType>::StandardJetPlots(TDirectory * tmpFile, TString tmpDirectory, TString tmpSubDirectory)
	{
		...
		charge = new TH1D("charge","charge of jet", 5, -2, 2);
	...
	void StandardJetPlots<JetType>::process(JetType * jet, KDataVertex * pv, double weight)
	{
		...
		charge->Fill(jet->charge, weight);
		n90->Fill(jet->n90, weight);
		nConstituents->Fill(jet->nConstituents, weight);
		emFraction->Fill(jet->emFraction, weight);
		hadFraction->Fill(jet->hadFraction, weight);
...
/*
	template <>
	void StandardJetPlots<MTAPFJet>::initSpecific()
	{
...
/*
	template <>
	void StandardJetPlots<MTAPFJet>::processSpecific(MTAPFJet * jet, KDataVertex * pv, double weight)
	{
		if (!jet)
			return;
...
//template class KappaTools::StandardJetPlots<MTAPFJet>;

/*
MTA::StandardPFJetsPlots::StandardPFJetsPlots(TDirectory * tmpFile, TString tmpDirectory, TString tmpSubDirectory) : StandardJetPlots(tmpFile, tmpDirectory, tmpSubDirectory)
{
	neutralEmFraction = new TH1D("neutralEmFraction", "neutral electromagnetic fraction", 50, 0., 1.);
	chargedEmFraction = new TH1D("chargedEmFraction", "charged electromagnetic fraction", 50, 0., 1.);
	neutralHadFraction = new TH1D("neutralHadFraction", "neutral electromagnetic fraction", 50, 0., 1.);
...
void MTA::StandardPFJetsPlots::process(MTAPFJet * jet, MTAVertex * pv)
{
	neutralEmFraction->Fill(jet->neutralEmFraction);
	chargedEmFraction->Fill(jet->chargedEmFraction);
	neutralHadFraction->Fill(jet->neutralHadFraction);
	chargedHadFraction->Fill(jet->chargedHadFraction);

	StandardJetPlots::process(jet, pv);
}

void MTA::StandardPFJetsPlots::final()
{
	StandardJetPlots::final();
}
*/
